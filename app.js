...
(function () {
  ...
  function parseDuration(str) {
    ...
    if (colonParts.length === 2) {
      return (
        (parseFloat(colonParts[0]) || 0) + (parseFloat(colonParts[1]) || 0) / 60
      );
    }
  ...
  function navigate(view, params) {
    ...
    var _pickerHeader =
      $("slot-picker-sheet") &&
      $("slot-picker-sheet").querySelector(".sheet-header span");
  ...
  function segmentFormHTML(seg, idx, total, isMultiDay, startDate, endDate) {
    ...
    return (
      ...
      "<label>Name</label>" +
      '<input class="form-input seg-name" type="text" value="' +
      escHtml(seg.name) +
      ...
      (isDaily
        ? '<div class="form-row">' +
          '<div class="form-group">' +
          "<label>Date</label>" +
          '<input class="form-input seg-date" type="date" value="' +
          escHtml(seg.date || "") +
          '" readonly>' +
          "</div>" +
          "</div>" +
          ...
          '<input class="form-input seg-carbs-target" type="number" min="0" value="' +
          seg.targets.carbsPerHour +
          '">' +
          "</div>" +
          '<div class="form-group">' +
          "<label>Sodium/day (mg)</label>" +
          '<input class="form-input seg-sodium-target" type="number" min="0" value="' +
          seg.targets.sodiumPerHour +
          '">' +
          "</div>" +
          "</div>"
        : '<div class="form-row">' +
          ...
          '<input class="form-input seg-duration" type="text" inputmode="text" value="' +
          formatDuration(seg.durationHours) +
          '" placeholder="e.g. 1:45 or 1h45m">' +
          "</div>" +
          "</div>" +
          '<div class="form-row">' +
          '<div class="form-group">' +
          ...
          '<input class="form-input seg-caffeine-target" type="number" min="0" value="' +
  ...
  function eventCardHTML(evt) {
    ...
    return (
      ...
      (EVENT_TYPE_LABELS[evt.type] || escHtml(evt.type)) +
      "</span>" +
      "</div>" +
  ...
  async function renderEventsList() {
    ...
    function injectEventsFab() {
      ...
      viewEl.insertAdjacentHTML(
        "beforeend",
        '<button class="fl-fab" id="events-fab" aria-label="New event"><i class="ti ti-plus"></i></button>',
      );
      on($("events-fab"), "click", function () {
        navigate("create", { currentEventId: null, currentEvent: null });
      });
    }

    if (!events.length) {
      $list.innerHTML =
        '<div class="empty-state"><div style="font-size:48px">🚴</div><p>No events yet.</p><p>Tap + to plan your first one.</p></div>';
      injectEventsFab();
      _refreshClaimIndicator();
      return;
    }
    ...
    upcoming.forEach(function (evt) {
      (evt.segments || []).forEach(function (seg) {
        (seg.items || []).forEach(function (item) {
          ...
          if (!fuelTotals[item.name])
            fuelTotals[item.name] = { qty: 0, type: item.type };
    ...
    if (fuelEntries.length) {
      html +=
        '<details class="fuel-totals-section">' +
        '<summary class="fuel-totals-summary">' +
        '<span class="fuel-label-text">Fuel running total</span>' +
        '<span class="fuel-count-chip">' +
        fuelEntries.length +
        (fuelEntries.length === 1 ? " product" : " products") +
        "</span>" +
        "</summary>" +
        ...
        fuelEntries
          .map(function (e) {
            return (
              '<div class="fuel-row">' +
              '<span class="fuel-row-name">' +
              escHtml(e.name) +
              ' <span class="fuel-row-type">' +
              escHtml(e.typeLabel) +
              "</span></span>" +
              '<span class="fuel-row-qty">× ' +
              formatQty(e.qty) +
              "</span>" +
              "</div>"
            );
    ...
    if (past.length) {
      html +=
        ...
        '<summary class="past-events-summary">Past events (' +
        past.length +
        ")</summary>" +
  ...
  function _refreshClaimIndicator() {
    ...
    if (
      !hideOnView &&
      !_indicatorDismissed &&
      localStorage.getItem("fuelPlanner.isAnonymous") === "true"
    ) {
  ...
  function _syncCarbLoadSegments() {
    ...
    if (!startDate) return;
    var endDate =
      $("ef-category").value === "multi"
        ? $("ef-end-date").value || startDate
        : startDate;
  ...
  function syncDraftSegmentsFromDOM() {
    ...
    segCards.forEach(function (card) {
      ...
      var seg = draftSegments.find(function (s) {
        return s.id === id;
      });
      if (!seg) return;
          ...
          if (
            !confirm(
              'Remove segment "' +
                ...
                '"? It has ' +
                seg.items.length +
                " item(s) which will be lost.",
            )
          )
            return;
        }
        draftSegments = draftSegments.filter(function (s) {
          return s.id !== id;
        });
        renderSegmentForms();
      });
  ...
  on($("event-form"), "submit", async function (e) {
    ...
    var segments = Array.from(segCards).map(function (card) {
      var id = card.dataset.segDraftId;
      var existing = draftSegments.find(function (s) {
        return s.id === id;
      });
      ...
      return {
        ...
        durationHours: isDaily
          ? 0
          : durEl
            ? parseDuration(durEl.value) || 1
            : 1,
        targets: {
          carbsPerHour:
            parseFloat(card.querySelector(".seg-carbs-target").value) || 0,
          sodiumPerHour:
            parseFloat(card.querySelector(".seg-sodium-target").value) || 0,
          caffeinePerHour: isDaily
            ? 0
            : caffEl
              ? parseFloat(caffEl.value) || 0
              : 0,
        },
    ...
    if (isEdit) {
      var base;
      try {
        base = await Data.getEvent(state.currentEventId);
      } catch (e) {
        showToast("Couldn't load event — check your connection.");
        return;
      }
  ...
  function metricCardHTML(key, value, rate, actualValue, actualRate, goalRate) {
    ...
    return (
      ...
      (hasActuals ? " has-actuals" : "") +
      '">' +
      '<div class="metric-value">' +
      value +
      "</div>" +
      (hasActuals
        ? '<div class="metric-actual">' + actualValue + "</div>"
        : "") +
      ...
      "</div>" +
      (hasActuals
        ? '<div class="metric-actual-rate">' + actualRate + "</div>"
        : "") +
      (hasActuals && goalRate
        ? '<div class="metric-goal-rate">🎯 ' + goalRate + "</div>"
        : "") +
      "</div>"
    );
  }
  ...
  function segmentSectionHTML(seg, showLabel, foodLogActuals) {
    ...
    if (isDaily) {
      ...
      var pctPlannedCarbs = dailyCarbTarget
        ? Math.min((totals.carbs / dailyCarbTarget) * 100, 150)
        : 0;
      var pctPlannedSodium = dailySodiumTarget
        ? Math.min((totals.sodium / dailySodiumTarget) * 100, 150)
        : 0;
      ...
      var actuals =
        foodLogActuals && seg.date ? foodLogActuals[seg.date] || null : null;
      var foodLogBlock = "";
      if (foodLogActuals !== undefined) {
        ...
        var pctActCarbs = dailyCarbTarget
          ? Math.min((actCarbs / dailyCarbTarget) * 100, 150)
          : 0;
        var pctActSodium = dailySodiumTarget
          ? Math.min((actSodium / dailySodiumTarget) * 100, 150)
          : 0;
        ...
        foodLogBlock =
          ...
          "</span>" +
          (!isFutureDay
            ? '<a href="#" class="food-log-actuals-link" data-navigate-food-log="' +
              escHtml(seg.date || "") +
              '">View ›</a>'
            : "") +
          "</div>" +
          (actuals && !isFutureDay
            ? '<div class="progress-group">' +
              progressRowHTML(
                "Carbs",
                actCarbs + "g / " + dailyCarbTarget + "g",
                pctActCarbs,
                stActCarbs,
              ) +
              progressRowHTML(
                "Sodium",
                actSodium + "mg / " + dailySodiumTarget + "mg",
                pctActSodium,
                stActSodium,
              ) +
              "</div>"
      ...
      return (
        '<div class="segment-section" data-segment-id="' +
        seg.id +
        '">' +
        (showLabel
          ? '<div class="segment-header">' +
            '<div class="segment-title-row">' +
            '<span class="segment-name editable" data-inline="seg-name">' +
            escHtml(seg.name) +
            "</span>" +
            (seg.date
            ...
            '<span class="target-pill" data-inline="seg-carbs-target">' +
            dailyCarbTarget +
            "g carbs/day</span>" +
            '<span class="target-pill" data-inline="seg-sodium-target">' +
            dailySodiumTarget +
            "mg Na/day</span>" +
            "</div>" +
            "</div>"
        ...
        progressRowHTML(
          "Carbs",
          totals.carbs + "g / " + dailyCarbTarget + "g",
          pctPlannedCarbs,
          stPlannedCarbs,
        ) +
        progressRowHTML(
          "Sodium",
          totals.sodium + "mg / " + dailySodiumTarget + "mg",
          pctPlannedSodium,
          stPlannedSodium,
        ) +
        "</div>" +
    ...
    var pctCarbs = tgt.carbsPerHour
      ? Math.min((rates.carbs / tgt.carbsPerHour) * 100, 150)
      : 0;
    var pctSodium = tgt.sodiumPerHour
      ? Math.min((rates.sodium / tgt.sodiumPerHour) * 100, 150)
      : 0;
    var pctCaff = tgt.caffeinePerHour
      ? Math.min((rates.caffeine / tgt.caffeinePerHour) * 100, 150)
      : 0;
    ...
    return (
      ...
      (showLabel
        ? '<div class="segment-header">' +
          ...
          escHtml(seg.name) +
          "</span>" +
          '<span class="segment-duration editable" data-inline="seg-duration">&nbsp;· ' +
          dhLabel +
          "</span>" +
          "</div>" +
          '<div class="segment-targets-row">' +
          ...
          "mg Na/hr</span>" +
          (tgt.caffeinePerHour
            ? '<span class="target-pill" data-inline="seg-caff-target">' +
              tgt.caffeinePerHour +
              "mg caff/hr</span>"
            : "") +
          "</div>" +
      ...
      progressRowHTML(
        "Sodium",
        fmt(rates.sodium, "mg/hr"),
        pctSodium,
        stSodium,
      ) +
      progressRowHTML(
        "Caffeine",
        fmt(rates.caffeine, "mg/hr"),
        pctCaff,
        stCaff,
      ) +
      "</div>" +
  ...
  function executionPlanHTML(seg) {
    ...
    var segInterval =
      seg.execInterval && seg.execInterval > 0
        ? seg.execInterval
        : hasPlan
          ? planInterval
    ...
    var expectedSlots = hasPlan
      ? Math.floor(((seg.durationHours || 1) * 60) / planInterval) + 1
      : 0;
    var isStale =
      hasPlan &&
      (plan.length !== expectedSlots || planInterval !== segInterval);

    // Check for orphaned itemIds (items removed since generation)
    var itemIds = new Set(
      (seg.items || []).map(function (i) {
        return i.id;
      }),
    );
    var hasOrphans =
      hasPlan &&
      plan.some(function (slot) {
        return (slot.assignments || []).some(function (a) {
          if (a.type === "drink_group")
            return (a.itemIds || []).some(function (id) {
    ...
    if (hasPlan) {
      plan.forEach(function (slot) {
        (slot.assignments || []).forEach(function (a) {
          if (a.type === "drink_group") {
            (a.itemIds || []).forEach(function (id) {
              planItemIds.add(id);
            });
          } else {
            planItemIds.add(a.itemId);
          }
        });
      });
    }
    var hasNewItems =
      hasPlan &&
      (seg.items || []).some(function (i) {
        return !planItemIds.has(i.id);
      });
    ...
    var headerHTML =
      ...
      '<button class="exec-plan-toggle" data-exec-toggle="' +
      seg.id +
      '" aria-expanded="false">' +
      ...
      '<span class="exec-plan-interval editable" data-inline="exec-interval">' +
      segInterval +
      " min</span>" +
      "</div>" +
      (hasPlan
        ? '<div class="exec-plan-header-actions">' +
          '<button class="exec-plan-regen-btn" data-exec-regen="' +
          seg.id +
          '">Regenerate</button>' +
          '<button class="exec-plan-delete-btn" data-exec-delete="' +
          seg.id +
          '">Delete</button>' +
          "</div>"
        : '<button class="exec-plan-generate-btn" data-exec-generate="' +
          seg.id +
          '">Generate</button>') +
      "</div>";
    ...
    if (!hasPlan) {
      return (
        '<div class="exec-plan-panel" data-exec-panel="' +
        seg.id +
        '">' +
        headerHTML +
        "</div>"
    ...
    var slotsHTML = plan
      .map(function (slot) {
        ...
        var carbLabel =
          slotCarbs > 0
            ? '<span class="exec-slot-carbs">~' +
              Math.round(slotCarbs) +
              "g</span>"
            : "";

        var assignmentLabels = (slot.assignments || [])
          .map(function (a) {
            if (a.type === "drink_group") {
              return a.groupName ? "Sip " + escHtml(a.groupName) : "Sip";
            }
            var item = (seg.items || []).find(function (i) {
              return i.id === a.itemId;
            });
            if (!item) return "";
            var fullName = escHtml(
        ...
        return (
          '<div class="exec-slot-row' +
          (isEmpty ? " exec-slot-empty" : "") +
          '" ' +
          'data-exec-slot="' +
          seg.id +
          ":" +
          slot.slotIndex +
          '">' +
          '<span class="exec-slot-time">' +
          slotTimeLabel(slot.slotIndex, slot.intervalMinutes) +
          "</span>" +
          '<span class="exec-slot-items">' +
          (isEmpty ? "—" : assignmentLabels) +
          "</span>" +
          carbLabel +
          "</div>"
        );
    ...
    var planCarbsPerHr =
      seg.durationHours > 0 ? planTotalCarbs / seg.durationHours : 0;
    var targetCarbs =
      seg.targets && seg.targets.carbsPerHour ? seg.targets.carbsPerHour : 0;
    var footerClass =
      targetCarbs > 0 && planCarbsPerHr < targetCarbs * 0.85
        ? " exec-plan-footer-warn"
        : "";
    var footerHTML =
      '<div class="exec-plan-footer' +
      footerClass +
      '">' +
      "~" +
      Math.round(planTotalCarbs) +
      "g carbs total · " +
      "~" +
      Math.round(planCarbsPerHr) +
      "g/hr" +
      (targetCarbs > 0
        ? ' <span class="exec-plan-footer-target">(target: ' +
          targetCarbs +
          "g/hr)</span>"
        : "") +
      "</div>";
    ...
    return (
      ...
      '">' +
      headerHTML +
      '<div class="exec-plan-body hidden" data-exec-body="' +
      seg.id +
      '">' +
      staleHTML +
      '<div class="exec-slots-list">' +
      slotsHTML +
      "</div>" +
      footerHTML +
      "</div>" +
  ...
  function progressRowHTML(label, value, pct, status) {
    return (
      ...
      label +
      "</span>" +
      '<div class="progress-track"><div class="progress-fill" style="--pct:' +
      pct.toFixed(1) +
      '"></div></div>' +
  ...
  function formatQty(q) {
    ...
    if (frac > 0) {
      var closest = FRACTIONS.reduce(function (best, f) {
        return Math.abs(f.value - frac) < Math.abs(best.value - frac)
          ? f
          : best;
      });
  ...
  function itemRowHTML(item) {
    ...
    return (
      ...
      '<span class="stepper-qty qty-frac-trigger" data-item-id="' +
      item.id +
      '">' +
  ...
  function actualItemRowHTML(item) {
    ...
    return (
      ...
      '<div class="stepper">' +
      '<button class="stepper-btn" data-action="dec"><i class="ti ti-minus"></i></button>' +
      '<span class="stepper-qty qty-actual-frac-trigger">' +
      formatQty(item.quantity) +
      "</span>" +
      '<button class="stepper-btn" data-action="inc"><i class="ti ti-plus"></i></button>' +
      "</div>" +
  ...
  function postEventNotesHTML(notes) {
    return (
      '<div class="post-event-notes-section">' +
  ...
  function actualSegmentSectionHTML(seg, actualSeg, isMultiDay) {
    ...
    if (dh && dh > 0) {
      ...
      var tgt = seg.targets;
      var pctCarbs = tgt.carbsPerHour
        ? Math.min((actRates.carbs / tgt.carbsPerHour) * 100, 150)
        : 0;
      var pctSodium = tgt.sodiumPerHour
        ? Math.min((actRates.sodium / tgt.sodiumPerHour) * 100, 150)
        : 0;
      var pctCaff = tgt.caffeinePerHour
        ? Math.min((actRates.caffeine / tgt.caffeinePerHour) * 100, 150)
        : 0;
      ...
      gaugesHTML =
        ...
        progressRowHTML(
          "Carbs",
          fmt(actRates.carbs, "g/hr"),
          pctCarbs,
          stCarbs,
        ) +
        progressRowHTML(
          "Sodium",
          fmt(actRates.sodium, "mg/hr"),
          pctSodium,
          stSodium,
        ) +
        progressRowHTML(
          "Caffeine",
          fmt(actRates.caffeine, "mg/hr"),
          pctCaff,
          stCaff,
        ) +
        "</div>";
    ...
    return (
      '<div class="actual-section" data-actual-segment-id="' +
      seg.id +
      '">' +
      '<div class="actual-section-header">' +
      '<span class="actual-pill">ACTUAL</span>' +
      '<span class="actual-section-title">' +
      escHtml(seg.name) +
      "</span>" +
      "</div>" +
  ...
  function attachDetailHandlers(evt) {
    ...
    $("btn-detail-back").onclick = function () {
      navigate("events");
    };
    ...
    $$(".qty-frac-trigger", $("detail-body")).forEach(function (el) {
      ...
      attachQtyFracTrigger(
        el,
        evt.id,
        segSection.dataset.segmentId,
        row.dataset.itemId,
      );
    });
    ...
    $$("[data-add-actual-segment-id]", $("detail-body")).forEach(
      function (btn) {
        on(btn, "click", function () {
    ...
    $$(
      '[data-inline]:not([data-inline="actual-duration"])',
      $("detail-body"),
    ).forEach(function (el) {
    ...
    $$(".actual-section .stepper-btn", $("detail-body")).forEach(
      function (btn) {
        on(btn, "click", function () {
    ...
    // Actual qty fraction triggers
    $$(".qty-actual-frac-trigger", $("detail-body")).forEach(function (el) {
      var row = el.closest("[data-actual-item-id]");
      var secEl = el.closest("[data-actual-segment-id]");
      if (!row || !secEl) return;
      attachActualQtyFracTrigger(
        el,
        evt.id,
        secEl.dataset.actualSegmentId,
        row.dataset.actualItemId,
      );
    });

    // Actual duration inline edit
    $$('[data-inline="actual-duration"]', $("detail-body")).forEach(
      function (el) {
        on(el, "click", function () {
          var secEl = el.closest("[data-actual-segment-id]");
          if (!secEl) return;
          ...
          var dh =
            ...
            state.currentEvent.actuals[segId]
              ? state.currentEvent.actuals[segId].durationHours
              : null;
          ...
          makeEditable(el, async function (val) {
            try {
              ...
              if (!evt2.actuals[segId])
                evt2.actuals[segId] = { durationHours: null, items: [] };
              evt2.actuals[segId].durationHours = num;
              await Data.saveActuals(
                evt2.id,
                evt2.actuals,
                evt2.postEventNotes,
              );
    ...
    if (sharePlanBtn) {
      on(sharePlanBtn, "click", function () {
        ...
        var md = Export.generateEventMarkdown(
          evt,
          Object.keys(execPlans).length ? execPlans : null,
        );
        if (navigator.share) {
    ...
    $$("[data-exec-generate]", $("detail-body")).forEach(function (btn) {
      on(btn, "click", function () {
        handleExecutionPlanGenerate(btn.dataset.execGenerate, false);
      });
    });
    $$("[data-exec-regen]", $("detail-body")).forEach(function (btn) {
      on(btn, "click", function () {
        handleExecutionPlanGenerate(btn.dataset.execRegen, true);
      });
    });
    $$("[data-exec-toggle]", $("detail-body")).forEach(function (btn) {
      on(btn, "click", function () {
        handleExecutionPlanToggle(btn.dataset.execToggle);
      });
    });
  ...
  function refreshSummaryCards() {
    ...
    var archiveDate =
      evt.category === "multi" && evt.endDate ? evt.endDate : evt.date;
    var showAct =
      isEventPastOrToday(archiveDate) &&
      Object.keys(evt.actuals || {}).length > 0;
    ...
    var footerEl =
      $("detail-body") && $("detail-body").querySelector(".totals-footer");
    if (footerEl) {
      var totalH = totals.durationHours;
  ...
  function reattachSegmentHandlers(segId) {
    ...
    $$('[data-inline]:not([data-inline="actual-duration"])', segEl).forEach(
      function (el) {
        on(el, "click", function () {
          handleInlineEdit(el, evt.id);
        });
    ...
    if (deleteBtn) {
      on(deleteBtn, "click", function () {
        if (!confirm("Delete this execution plan? This cannot be undone."))
          return;
        var segId = deleteBtn.dataset.execDelete;
        Data.deleteExecutionPlan(segId);
        var multiSeg = evt.segments.length > 1;
        var seg2 = (evt.segments || []).find(function (s) {
          return s.id === segId;
        });
        if (seg2) {
  ...
  function reattachActualSegmentHandlers(segId) {
    ...
    $$(".qty-actual-frac-trigger", secEl).forEach(function (el) {
      var row = el.closest("[data-actual-item-id]");
      if (!row) return;
      attachActualQtyFracTrigger(el, evt.id, segId, row.dataset.actualItemId);
    });
  ...
  function handleExecutionPlanGenerate(segId, forceRegenerate) {
    var evt = state.currentEvent;
    if (!evt) return;
    var seg = (evt.segments || []).find(function (s) {
      return s.id === segId;
    });
    if (!seg) return;
    ...
    if (forceRegenerate && Data.loadExecutionPlan(segId)) {
      if (!confirm("Regenerate plan? Any manual edits will be replaced."))
        return;
    }
  ...
  function openSlotEditor(segId, slotIndex) {
    ...
    $("slot-editor-title").textContent = slotTimeLabel(
      slotIndex,
  ...
  function attachQtyFracTrigger(el, eventId, segId, itemId) {
    on(el, "click", function (e) {
      ...
      if (!evt2) return;
      var seg2 = (evt2.segments || []).find(function (s) {
        return s.id === segId;
      });
      if (!seg2) return;
      var item2 = (seg2.items || []).find(function (i) {
        return i.id === itemId;
      });
      if (!item2) return;
      var whole = Math.floor(item2.quantity);
      var opts = (
        whole > 0 ? [{ label: String(whole), value: whole }] : []
      ).concat(
        FRACTIONS.map(function (f) {
          return {
            label: (whole > 0 ? whole : "") + f.label,
            value: whole + f.value,
          };
        }),
      );
      ...
      opts.forEach(function (opt) {
        ...
        btn2.className =
          "qty-frac-option" +
          (Math.abs(item2.quantity - opt.value) < 0.01 ? " selected" : "");
        ...
        on(btn2, "click", function (e2) {
          ...
          if (Math.abs(delta) > 0.001)
            updateItemQty(eventId, segId, itemId, delta);
        });
  ...
  function attachActualQtyFracTrigger(el, eventId, segId, itemId) {
    on(el, "click", function (e) {
      e.stopPropagation();
      closeQtyDropdown();
      var evt2 = state.currentEvent;
      if (!evt2 || !evt2.actuals) return;
      var actualSeg = evt2.actuals[segId];
      if (!actualSeg) return;
      var item2 = (actualSeg.items || []).find(function (i) {
        return i.id === itemId;
      });
      if (!item2) return;
      var whole = Math.floor(item2.quantity);
      ...
      opts.forEach(function (opt) {
        ...
        on(btn2, "click", function (e2) {
          e2.stopPropagation();
          closeQtyDropdown();
          if (Math.abs(item2.quantity - opt.value) > 0.001) {
            updateActualItemQty(
              eventId,
              segId,
              itemId,
              opt.value - item2.quantity,
            );
          }
        });
        dropdown.appendChild(btn2);
  ...
  function renderSlotEditorBody(seg, slot) {
    ...
    if (!slot.assignments || !slot.assignments.length) {
      bodyEl.innerHTML =
        '<div class="slot-empty-label">No items assigned. Tap + Add item below.</div>';
      return;
    }

    bodyEl.innerHTML = slot.assignments
      .map(function (a, aIdx) {
        var label;
        if (a.type === "drink_group") {
        ...
        return (
          '<div class="slot-assignment-row" data-assignment-idx="' +
          aIdx +
          '">' +
          '<span class="slot-assignment-label">' +
          label +
          "</span>" +
          '<div class="slot-assignment-actions">' +
          '<button data-slot-move-idx="' +
          aIdx +
          '">Move</button>' +
          '<button data-slot-remove-idx="' +
          aIdx +
          '">Remove</button>' +
          "</div>" +
          "</div>"
    ...
    $$("[data-slot-remove-idx]", bodyEl).forEach(function (btn) {
      on(btn, "click", function () {
        ...
        refreshSlotEditorAndPanel(
          _slotEditorSegId,
          _slotEditorSlotIndex,
          seg,
          plan,
        );
      });
      ...
      .map(function (slot, idx) {
        ...
        var summary =
          (slot.assignments || [])
            .map(function (a) {
              if (a.type === "drink_group")
                return a.groupName ? escHtml(a.groupName) : "Sip";
              var item = itemMap[a.itemId];
              return item
                ? escHtml((item.brand ? item.brand + " " : "") + item.name)
                : "";
        ...
        return (
          '<div class="slot-picker-row" data-pick-slot="' +
          idx +
          '">' +
          '<span class="slot-picker-time">' +
          slotTimeLabel(idx, interval) +
          "</span>" +
          '<span class="slot-picker-summary">' +
          summary +
          "</span>" +
          "</div>"
        );
  ...
  function openSlotAddItemPicker(seg, slotIndex) {
    ...
    pickerBody.innerHTML = (seg.items || [])
      .map(function (item) {
        var label = escHtml((item.brand ? item.brand + " " : "") + item.name);
        var remainLabel;
        if (item.type === "drink_powder" || item.type === "liquid") {
          remainLabel = inLiquidMix.has(item.id)
            ? "in liquid mix"
            : "not yet in mix";
        } else {
          var totalAssigned = assignedQty[item.id] || 0;
          var remaining =
            Math.round((item.quantity - totalAssigned) * 100) / 100;
          remainLabel =
            remaining > 0 ? remaining + " remaining" : "fully assigned";
        }

        return (
          '<div class="slot-picker-row" data-add-item-id="' +
          item.id +
          '" data-add-item-type="' +
          escHtml(item.type) +
          '">' +
          '<span class="slot-picker-time" style="min-width:unset;flex:1">' +
          label +
          "</span>" +
          '<span class="slot-picker-summary" style="margin-left:0">' +
          remainLabel +
          "</span>" +
          "</div>"
        );
    ...
    $$("[data-add-item-id]", pickerBody).forEach(function (row) {
      on(row, "click", function () {
        ...
        if (itemType === "drink_powder" || itemType === "liquid") {
          ...
          var newItem = (seg.items || []).find(function (i) {
            return i.id === newItemId;
          });
          var addCarbs = newItem
            ? ((newItem.carbsPerUnit || 0) * (newItem.quantity || 0)) /
              freshPlan.length
            : 0;
          var addSodium = newItem
            ? ((newItem.sodiumPerUnit || 0) * (newItem.quantity || 0)) /
              freshPlan.length
            : 0;
          freshPlan.forEach(function (s) {
            var group = (s.assignments || []).find(function (a) {
              return a.type === "drink_group" && a.groupId === "__auto__";
            });
            if (group) {
              if (group.itemIds.indexOf(newItemId) === -1)
                group.itemIds.push(newItemId);
              group.carbsPerSlot =
                Math.round((group.carbsPerSlot + addCarbs) * 100) / 100;
              group.sodiumPerSlot =
                Math.round((group.sodiumPerSlot + addSodium) * 100) / 100;
            } else {
              s.assignments.push({
                type: "drink_group",
                groupId: "__auto__",
  ...
  function updateItemQty(eventId, segId, itemId, delta) {
    ...
    if (item.quantity === 0) {
      seg.items = seg.items.filter(function (i) {
        return i.id !== itemId;
      });
    }
  ...
  function updateActualItemQty(eventId, segId, itemId, delta) {
    ...
    if (!actualSeg) return;
    var item = actualSeg.items.find(function (i) {
      return i.id === itemId;
    });
    if (!item) return;
    ...
    if (item.quantity === 0) {
      actualSeg.items = actualSeg.items.filter(function (i) {
        return i.id !== itemId;
      });
    }
    ...
    // Save in background
    Data.saveActuals(evt.id, evt.actuals, evt.postEventNotes).catch(
      function () {
        showToast("Couldn't save — check your connection.");
      },
    );
  ...
  async function startActuals(eventId) {
    var evt;
    try {
      evt = await Data.getEvent(eventId);
    } catch (e) {
      showToast("Couldn't load event — check your connection.");
      return;
    }
  ...
  async function renderSheetLibraryTab(query) {
    ...
    if (!query && recent.length) {
      ...
      $("recent-products-list").innerHTML = recent
        .map(function (p) {
    ...
    if (query) {
      ...
      var sheetTypes = TYPE_ORDER.concat(
        ...
        return sheetGroups[t];
      });
      $results.innerHTML = sheetTypes
        .map(function (type) {
          return (
            '<div class="product-group-title" style="padding:8px 16px 4px;font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--text-tertiary)">' +
  ...
  on($("btn-slot-add-item"), "click", function () {
    var evt = state.currentEvent;
    if (!evt || _slotEditorSegId === null || _slotEditorSlotIndex === null)
      return;
    var seg = (evt.segments || []).find(function (s) {
      return s.id === _slotEditorSegId;
    });
    if (!seg) return;
  ...
  async function renderSettings() {
    ...
    if (existingTargetsSection)
      existingTargetsSection.parentNode.removeChild(existingTargetsSection);
    ...
    if (userId) {
      ...
      try {
        targets =
          (await (window.FoodLogData &&
            window.FoodLogData.getTargets(userId))) || {};
      } catch (e) {}
      var keys = [
        ...
        rowsEl.innerHTML = keys
          .map(function (f) {
            ...
            return (
              ...
              f.label +
              "</span>" +
              '<input class="fl-targets-input" type="number" min="0" data-tkey="' +
              f.key +
              '" value="' +
              val +
              '" placeholder="' +
        ...
        rowsEl2.addEventListener(
          ...
          async function (e) {
            ...
            $$("[data-tkey]").forEach(function (i) {
              var v = i.value.trim();
              updated[i.dataset.tkey] =
                v !== "" ? Math.max(0, parseFloat(v) || 0) : null;
            });
            try {
    ...
    if (planSection) {
      ...
      planSection.innerHTML =
        ...
        '<input id="settings-exec-interval" class="fl-targets-input" type="number" min="1" max="120" value="' +
        curInterval +
        '">' +
        "</div>" +
        "</div>" +
        ...
        on(intervalInput, "blur", function () {
          ...
          if (!isNaN(n) && n > 0) {
            ...
            intervalInput.value = String(
              Math.max(1, Data.getDefaultExecInterval()),
            );
          }
        });
      }
    ...
    if (isAnonymous) {
      ...
      on($("btn-settings-new-phrase"), "click", function () {
        if (
          !confirm(
            "This will replace your current recovery phrase. Your old phrase will stop working immediately. Make sure you're ready to save the new one before continuing.",
          )
        )
          return;
        state.claimReturnView = "settings";
  ...
  function handleInlineEdit(el, eventId) {
    ...
    if (field === "seg-duration") {
      ...
      var curSeg =
        state.currentEvent &&
        state.currentEvent.segments.find(function (s) {
          return s.id === segId;
        });
      ...
      var curInterval =
        curSeg2 && curSeg2.execInterval && curSeg2.execInterval > 0
  ...
  async function init() {
    ...
    // Recovery screen
    on($("btn-recovery-back"), "click", function () {
      navigate("landing");
    });
    ...
    on($("btn-find-data"), "click", async function () {
      ...
      try {
        ...
        if (userId) {
          ...
          if (errorEl) {
            errorEl.textContent =
              "No data found for this phrase. Check for typos and try again.";
            errorEl.classList.remove("hidden");
          }
          btn.disabled = false;
          btn.textContent = "Find my data";
    ...
    Data.touchLastVisited().catch(function (e) {
      console.warn("touchLastVisited failed:", e);
    });
    ...
    try {
      ...
      showToast(
        "Migration failed \u2014 your local data is safe. Will retry next load.",
      );
    }
